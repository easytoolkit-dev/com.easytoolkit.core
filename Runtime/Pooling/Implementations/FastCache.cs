...
namespace EasyToolkit.Core.Pooling.Implementations
{
    ...
    internal sealed class FastCache<T> where T : class
    {
        /// <summary>
        ...
        public bool TryPut(T item)
        {
            ...
            if (ReferenceEquals(_slot1, item) && _slot1Idle == false)
            {
                _slot1Idle = true;
                return true;
            }

            if (ReferenceEquals(_slot2, item) && _slot2Idle == false)
            {
                _slot2Idle = true;
                return true;
            }

            if (ReferenceEquals(_slot3, item) && _slot3Idle == false)
            {
                _slot3Idle = true;
                return true;
            }
        ...
        /// <summary>
        /// Recreates all slots and clears the cache state.
        /// </summary>
        public void Clear()
        {
            RecreateSlots();
