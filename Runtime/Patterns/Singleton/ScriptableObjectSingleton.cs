            ...
            get
            {
                if (s_instance is null)
                {
                    lock (InitialLock)
                    {
                        ...
                        {
                            ScriptableObjectLoadMode.Memory => Patterns.ScriptableObjectFactory.CreateInMemory<T>(),
                            ScriptableObjectLoadMode.Asset => Patterns.ScriptableObjectFactory.LoadAssetOrFallback<T>(
                                config.AssetDirectory,
                                config.AssetName,
                                fallbackToMemory: false),
                            ScriptableObjectLoadMode.TryLoadAssetOrFallback => Patterns.ScriptableObjectFactory.LoadAssetOrFallback<T>(
                                config.AssetDirectory,
                                config.AssetName,
