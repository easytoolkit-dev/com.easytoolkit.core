    ...
    public sealed class DynamicExpressionEvaluator : ExpressionEvaluatorBase
    {
        ...
        private Func<object, object> CreateCompiledGetter([CanBeNull] Type rootType, [CanBeNull] Type sourceType, string path)
        {
            if (rootType != null)
            {
                ...
                var getter = new AccessorBuilder(path).BuildStaticGetter(rootType);
                return o => getter();
            }
            ...
            try
            {
                var getter = new AccessorBuilder(path).BuildInstanceGetter(sourceType);
                return o => getter(o);
            }
            catch (ArgumentException e)
            {
                ...
                try
                {
                    var getter = new AccessorBuilder(path).BuildStaticGetter(sourceType);
                    return o => getter();
                }
                catch (Exception)
                {
            ...
            string methodName, object[] arguments)
        {
            ...
            {
                // Static method call
                var invoker = new InvokerBuilder(methodName).BuildStaticFunc(rootType, parameterTypes);
            ...
            try
            {
                var invoker = new InvokerBuilder(methodName).BuildInstanceFunc(sourceType, parameterTypes);
            ...
            catch (ArgumentException e)
            {
                ...
                try
                {
                    var invoker = new InvokerBuilder(methodName).BuildStaticFunc(sourceType, parameterTypes);
                    return o => invoker(arguments);
                }
                catch (Exception)
                {
